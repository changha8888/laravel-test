...
function getDisplayName($uinfo){
    return isset($uinfo['displayname']) && !empty($uinfo['displayname']) ? $uinfo['displayname'] : Common::maskPhone($uinfo['phone']);
}

function getFullDisplayName($uinfo){
    return isset($uinfo['displayname']) && !empty($uinfo['displayname'])  ? $uinfo['displayname'] : $uinfo['phone'];
}
...
function dequy($data, $p=0, $str="--", $select=0){
    foreach ($data as $key=>$val){
        $id = $val["id"];
        $name = $val["name"];
